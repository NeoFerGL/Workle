...
class CypherUtils:
    ...
    def encripta(plaintext):
        ...
        enc_bytes = CypherUtils.encrypt1(
    ...
    def desencripta(cyphertext):
        password = settings.CRYPTO_PASSWORD
        denc_bytes = CypherUtils.decrypt1(
            cyphertext.encode('utf-8'), password.encode('utf-8'))
        return denc_bytes.decode('utf-8')

    @staticmethod
    def cipherFernet(password):
        key = PBKDF2HMAC(algorithm=hashes.SHA256(), length=32, salt=b'hdjk',
